...
dir_kernels <- "/Users/ameydanole/Desktop/ENS_Rennes/argh/Microplastic_ingestion_by_fulmarus_glacialis/amt/outputs"
...
range(indiv_merged_df$timestamp)
...
for(i in unique(nbs_mylocs$colony)){ # First for loop start
  ...
  if(nrow(sub) > 4){
    ...
    # Kernel estimation
    dat.track <- make_track(sub, lon, lat, timestamp, crs = DgProj, all_cols = T)
